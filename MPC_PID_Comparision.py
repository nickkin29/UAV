...
import support_files_drone_mixed_constraints as sfd_constraints
...
statesTotals_pid = [states_pid]

statesTotal_ani = [states[0:len(states)-6]]
statesTotal_ani_pid = [states_pid[0:len(states_pid)-6]]
...
ref_angles_total_pid = np.array([[phit, thetat, psit]])
...
omegas_bundle_pid = np.array([[omega1, omega2, omega3, omega4]])
UTotal_ani = UTotal
UTotal_ani_pid = UTotal_pid
...
for i_global in range(0, len(t) - 1):
    ...
    # PID Ref
    phi_ref_pid, theta_ref_pid, U1_pid = support.pos_controller(
    ...
    Psi_ref_pid = np.transpose([np.zeros(innerDyn_length + 1)])
    ...
    for yaw_step in range(0, innerDyn_length + 1):
        ...
        Psi_ref_pid[yaw_step] = psi_ref[i_global] + (psi_ref[i_global + 1] - psi_ref[i_global]) / (Ts * innerDyn_length) * Ts * yaw_step
    ...
    temp_angles_pid = np.concatenate((Phi_ref_pid[1:len(Phi_ref_pid)], Theta_ref_pid[1:len(Theta_ref_pid)], Psi_ref_pid[1:len(Psi_ref_pid)]), axis=1)
    ref_angles_total_pid = np.concatenate((ref_angles_total_pid, temp_angles_pid), axis=0)
    # Create a reference vector
    refSignals = np.zeros(len(Phi_ref) * controlled_states)
    refSignals_pid = np.zeros(len(Phi_ref_pid) * controlled_states)
    ...
    for i in range(0, len(refSignals), controlled_states):
        ...
        refSignals_pid[i] = float(Phi_ref_pid[k].item())
        refSignals_pid[i + 1] = float(Theta_ref_pid[k].item())
        refSignals_pid[i + 2] = float(Psi_ref_pid[k].item())
        k = k + 1
    ...
    for i in range(0, innerDyn_length):
        ...
        # PID control
        U2_pid, U3_pid, U4_pid = support.pid_controller((Phi_ref_pid[i][0]),
                                            (Theta_ref_pid[i][0]),
                                            (Psi_ref[i][0]),
                                            states_pid)
        ...
        if U1_pid > U1_max:
            U1_pid = U1_max
        U2_pid = np.clip(U2_pid, U2_min, U2_max)
        U3_pid = np.clip(U3_pid, U3_min, U3_max)
        U4_pid = np.clip(U4_pid, U4_min, U4_max)

        # Keep track of inputs
        UTotal_pid = np.concatenate((UTotal_pid, np.array([[U1_pid, U2_pid, U3_pid, U4_pid]])), axis = 0)

        U1C_pid = U1_pid / ct
        U2C_pid = U2_pid / (ct * l)
        U3C_pid = U3_pid / (ct * l)
        U4C_pid = U4_pid / cq
        UC_pid_vector = np.zeros((4, 1))
        UC_pid_vector[0, 0] = U1C_pid
        UC_pid_vector[1, 0] = U2C_pid
        UC_pid_vector[2, 0] = U3C_pid
        UC_pid_vector[3, 0] = U4C_pid

        omegas_vector_pid = np.matmul(omega_Matrix_inverse, UC_pid_vector)

        omega1P2_pid = omegas_vector_pid[0, 0]
        omega2P2_pid = omegas_vector_pid[1, 0]
        ...
        omega1_pid = np.clip(omega1_pid, omega_min, omega_max)
        omega2_pid = np.clip(omega2_pid, omega_min, omega_max)
        omega3_pid = np.clip(omega3_pid, omega_min, omega_max)
        omega4_pid = np.clip(omega4_pid, omega_min, omega_max)

        omegas_bundle = np.concatenate((omegas_bundle, np.array([[omega1, omega2, omega3, omega4]])), axis = 0)
        omegas_bundle_pid = np.concatenate((omegas_bundle_pid, np.array([[omega1_pid, omega2_pid, omega3_pid, omega4_pid]])), axis = 0)
        ...
        # PID
        states_pid, states_ani_pid, U_ani_pid = support.open_loop_new_states(states_pid, omega_total_pid, U1_pid, U2_pid, U3_pid, U4_pid)
        statesTotal_pid = np.concatenate((statesTotal_pid, [states_pid]), axis = 0)
        statesTotal_ani_pid = np.concatenate((statesTotal_ani_pid, states_ani_pid), axis = 0)
        UTotal_ani_pid = np.concatenate((UTotal_ani_pid, U_ani_pid), axis = 0)

    statesTotals = np.concatenate((statesTotals, [states]), axis = 0)
    statesTotals_pid = np.concatenate((statesTotals_pid, [states_pid]), axis = 0)
...
# PID Animation
statesTotal_x_pid=statesTotal_ani_pid[:,0]
statesTotal_y_pid=statesTotal_ani_pid[:,1]
statesTotal_z_pid=statesTotal_ani_pid[:,2]
statesTotal_phi_pid=statesTotal_ani_pid[:,3]
statesTotal_theta_pid=statesTotal_ani_pid[:,4]
statesTotal_psi_pid=statesTotal_ani_pid[:,5]
UTotal_U1_pid=UTotal_ani_pid[:,0]
UTotal_U2_pid=UTotal_ani_pid[:,1]
...
plt.subplot(3, 1, 1)
plt.plot(t, (X_ref - statesTotals_pid[:, 0]), 'g', linewidth=1, label='X(PID) Error')
plt.plot(t, (X_ref - statesTotals[:, 0]), 'r', linewidth=1, label='X(MPC) Error')
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('X Error', fontsize=15)
plt.legend(loc='upper right', fontsize="15")
plt.grid(True)

plt.subplot(3, 1, 2)
plt.plot(t, (Y_ref - statesTotals_pid[:, 1]), 'g', linewidth=1, label='Y(PID) Error')
plt.plot(t, (Y_ref - statesTotals[:, 1]), 'r', linewidth=1, label='Y(MPC) Error')
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Y Error', fontsize=15)
plt.legend(loc='upper right', fontsize="15")
plt.grid(True)

plt.subplot(3, 1, 3)
plt.plot(t, (Z_ref - statesTotals_pid[:, 2]), 'g', linewidth=1, label='Z(PID) Error')
plt.plot(t, (Z_ref - statesTotals[:, 2]), 'r', linewidth=1, label='Z(MPC) Error')
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Z Error', fontsize=15)
plt.legend(loc='upper right', fontsize="15")
plt.grid(True)
plt.show()
...
plt.plot(t_angles, statesTotal_pid[:, 0], 'g', linewidth=1, label='X(PID)')
plt.plot(t_angles, statesTotal[:, 0], 'r', linewidth=1, label='X(MPC)')
plt.xlabel('t (s)', fontsize=15)
...
plt.plot(t_angles, statesTotal_pid[:, 1], 'g', linewidth=1, label='Y(PID)')
plt.plot(t_angles, statesTotal[:, 1], 'r', linewidth=1, label='Y(MPC)')
plt.xlabel('t (s)', fontsize=15)
...
plt.plot(t_angles, statesTotal_pid[:, 2], 'g', linewidth=1, label='Z(PID)')
plt.plot(t_angles, statesTotal[:, 2], 'r', linewidth=1, label='Z(MPC)')
plt.xlabel('t (s)', fontsize=15)
...
plt.plot(t_angles, statesTotal[:, 3], 'r', linewidth=1, label='Phi(MPC)')
plt.xlabel('t (s)', fontsize=15)
...
plt.plot(t_angles, statesTotal[:, 4], 'r', linewidth=1, label='Theta(MPC)')
plt.xlabel('t (s)', fontsize=15)
...
plt.plot(t_angles, statesTotal[:, 5], 'r', linewidth=1, label='Psi(MPC)')
plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(3, 2, 2)
plt.plot(t_angles, ref_angles_total[:, 0], 'b', linewidth=1, label='Phi_ref')
plt.plot(t_angles, statesTotal_pid[:, 3], 'g', linewidth=1, label='Phi(PID)')
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Phi (rad)', fontsize=15)
plt.grid(True)
plt.legend(loc='upper right', fontsize="15")
...
plt.subplot(3, 2, 4)
plt.plot(t_angles, ref_angles_total[:, 1], 'b', linewidth=1, label='Theta_ref')
plt.plot(t_angles, statesTotal_pid[:, 4], 'g', linewidth=1, label='Theta(PID)')
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Theta (rad)', fontsize=15)
plt.grid(True)
plt.legend(loc='upper right', fontsize="15")
...
plt.subplot(3, 2, 6)
plt.plot(t_angles, ref_angles_total[:, 2], 'b', linewidth=1, label='Psi_ref')
plt.plot(t_angles, statesTotal_pid[:, 5], 'g', linewidth=1, label='Psi(PID)')
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Psi (rad)', fontsize=15)
plt.grid(True)
plt.legend(loc='upper right', fontsize="15")
...
plt.subplot(3, 2, 2)
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.plot(t, (phi_ref_pid - statesTotals_pid[:, 3]), 'g', linewidth=1, label='Phi(PID) Error')
plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(3, 2, 4)
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.plot(t, (theta_ref_pid - statesTotals_pid[:, 4]), 'g', linewidth=1, label='Theta(PID) Error ')
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Rad', fontsize=15)
plt.legend(loc='upper right', fontsize="15")
plt.grid(True)
...
plt.subplot(3, 2, 6)
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.plot(t, (Psi_ref_pid[0,:] - statesTotals_pid[:, 5]), 'g', linewidth=1, label='Psi(PID) Error ')
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Rad', fontsize=15)
plt.legend(loc='upper right', fontsize="15")
plt.grid(True)
...
plt.subplot(3, 2, 1)
plt.plot(t, (phi_ref - statesTotals[:, 3]), 'r', linewidth=1, label='Phi(MPC) Error')
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Rad', fontsize=15)
plt.legend(loc='upper right', fontsize="15")
...
plt.subplot(3, 2, 3)
plt.plot(t, (theta_ref - statesTotals[:, 4]), 'r', linewidth=1, label='Theta(MPC) Error')
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Rad', fontsize=15)
plt.legend(loc='upper right', fontsize="15")
...
plt.subplot(3, 2, 5)
plt.plot(t, (Psi_ref[0,:] - statesTotals[:, 5]), 'r', linewidth=1, label='Psi(MPC) Error')
plt.plot(t, np.zeros(126), 'b', linewidth=1)
plt.xlabel('t (s)', fontsize=15)
plt.ylabel('Rad', fontsize=15)
plt.legend(loc='upper right', fontsize="15")
...
plt.subplot(4, 1, 1)
plt.plot(t_angles, UTotal[0:len(UTotal), 0], 'r', linewidth=1, label='U1(MPC)')
plt.plot(t_angles, UTotal_pid[0:len(UTotal_pid), 0], 'g', linewidth=1, label='U1(PID)')
# plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(4, 1, 2)
plt.plot(t_angles, UTotal[0:len(UTotal), 1], 'r', linewidth=1, label='U2(MPC)')
plt.plot(t_angles, UTotal_pid[0:len(UTotal_pid), 1], 'g', linewidth=1, label='U2(PID)')
# plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(4, 1, 3)
plt.plot(t_angles, UTotal[0:len(UTotal), 2], 'r', linewidth=1, label='U3(MPC)')
plt.plot(t_angles, UTotal_pid[0:len(UTotal_pid), 2], 'g', linewidth=1, label='U3(PID)')
# plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(4, 1, 4)
plt.plot(t_angles, UTotal[0:len(UTotal), 3], 'r', linewidth=1, label='U4(MPC)')
plt.plot(t_angles, UTotal_pid[0:len(UTotal_pid), 3], 'g', linewidth=1, label='U4(PID)')
plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(4, 1, 1)
plt.plot(t_angles, omegas_bundle[0:len(omegas_bundle), 0], 'r', linewidth=1, label='omega 1(MPC)')
plt.plot(t_angles, omegas_bundle_pid[0:len(omegas_bundle_pid), 0], 'g', linewidth=1, label='omega 1(PID)')
# plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(4, 1, 2)
plt.plot(t_angles, omegas_bundle[0:len(omegas_bundle), 1], 'r', linewidth=1, label='omega 2(MPC)')
plt.plot(t_angles, omegas_bundle_pid[0:len(omegas_bundle_pid), 1], 'g', linewidth=1, label='omega 2(PID)')
# plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(4, 1, 3)
plt.plot(t_angles, omegas_bundle[0:len(omegas_bundle), 2], 'r', linewidth=1, label='omega 3(MPC)')
plt.plot(t_angles, omegas_bundle_pid[0:len(omegas_bundle_pid), 2], 'g', linewidth=1, label='omega 3(PID)')
# plt.xlabel('t (s)', fontsize=15)
...
plt.subplot(4, 1, 4)
plt.plot(t_angles, omegas_bundle[0:len(omegas_bundle), 3], 'r', linewidth=1, label='omega 4(MPC)')
plt.plot(t_angles, omegas_bundle_pid[0:len(omegas_bundle_pid), 3], 'g', linewidth=1, label='omega 4(PID)')
plt.xlabel('t (s)', fontsize=15)
