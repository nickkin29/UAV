...
# EKF Parameters
Q_pos = np.diag([0.01, 0.01, 0.01])
R_pos = np.diag([0.01, 0.01, 0.01])
P_pos = np.eye(3)
...
if no_plots == 1:
    ...
    ax.set_ylabel('Y [m]')
    ax.set_zlabel('Z [m]')
    plt.savefig("WithoutKF.png", dpi=300, bbox_inches="tight")
    ax.legend()
    plt.show()
    ...
    plt.savefig("Err_Without_KF", dpi=300, bbox_inches="tight")
